...
def fuzzy(fin,fout,col):
    ...
    try:
        ...
        for i in range(0,row,M): # 按行循环，读取文本文件
            ...
            mean = [0] * K
            e2 = [0] * K
            sheet.write(i // M + 1, K*2, df.iloc[i].values[0])  # 0是类别
            #sheet.cell(i//M+2, K*2+1).value = df.iloc[i].values[0]
            for m in range(M):
                for j in range(K):
                    sum[j] += df.iloc[i+m].values[j+1]
            mean[j] = sum[j]/M
            for m in range(M):
                for j in range(K):
                    e2[j] += (df.iloc[i+m].values[j+1]-mean[j])**2
            for j in range(K):
...
if __name__ == "__main__":
    fin = "../data/trip/data.xls"
    fout = "../data/trip/data-Inv.xls"
    col = 10  # attributes
    start = time.time()
    fuzzy(fin,fout,col)
    end = time.time()
