...
def L1_Distance(vector1, vector2):
	...
	dist = 0
	for i in range(K):
		dist += abs(vector2[0,2*i] - vector1[0,2*i])+abs(vector2[0,2*i+1] - vector1[0,2*i+1])
	return dist #求这两个矩阵的距离，vector1、2均为矩阵
...
def initCentroids(dataSet, k):
	...
	for i in range(k):
		index = random.randint(0, numSamples)  # 随机产生一个浮点数，然后将其转化为int型
...
def kmeans(dataSet, k):
	...
	clusterAssment = mat(np.zeros((numSamples, 2)))  # 得到一个N*2的零矩阵
	clusterChanged = True
	...
	while clusterChanged:
		...
		for i in range(numSamples):  # range
			...
			for j in range(k):
				distance = L1_Distance(centroids[j, :], dataSet[i, :])
				if distance < minDist:
					minDist = distance
					minIndex = j
...
if __name__ == "__main__":
	fin = "data/wall/data-S-Inv.xls"
	fout = "data/wall/data-S-Inv-labels.xls"
	df = pd.read_excel(fin)  # 导入信息
	xls1 = xlwt.Workbook()
	...
	dataSet = np.zeros((len(x),K))  # 列表，用来表示，列表中的每个元素也是一个二维的列表
	for i in range(len(x)):
		for j in range(0,K):
			lineArr = x[i][j]  # line.strip()把末尾的'\n'去掉
			dataSet[i][j]=float(lineArr)
	## step 2: clustering...
	print("step 2: clustering...")
	dataSet = mat(dataSet)  # mat()函数是Numpy中的库函数，将数组转化为矩阵
	centroids, clusterAssment = kmeans(dataSet, c)  # 调用KMeans文件中定义的kmeans方法。

	## step 3: show the result
	print("step 3: show the result...")
	#showCluster(dataSet, c, centroids, clusterAssment)
	sheet1.write(0, 0, 'predict')
	sheet1.write(0, 1, 'real')
	for i in range(len(clusterAssment)):
		sheet1.write(i + 1, 0, clusterAssment[i,0])
		sheet1.write(i + 1, 1, x[i][K])
	xls1.save(fout)
