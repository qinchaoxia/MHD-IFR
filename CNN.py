...
seed(1024)
import tensorflow
tensorflow.random.set_seed(1024)
'''
import numpy as np
import pandas as pd
import keras
...
# 载入数据
#df = pd.read_excel(r"data/wine/cnn/data-Inv.xls")
#df = pd.read_csv(r"data/wine/cnn/数据集-用做分类.csv")
df = pd.read_excel(r"data/iris/data-MIS.xls")
print(len(df))
K = 4
t = 3
X = np.expand_dims(df.values[:, 0:K].astype(float), axis=2)
Y = df.values[:, K]
...
def baseline_model():
    model = Sequential()
    model.add(Conv1D(4, 3, input_shape=(K, 1)))
    ...
    plot_model(model, to_file='data/model_classifier.png', show_shapes=True)
    print(model.summary())
...
def plot_confusion_matrix(cm, classes, title='Confusion matrix', cmap=plt.cm.jet):
    ...
    plt.ylabel('True Category', font)
    plt.xlabel('Predicted Category', font)
    plt.savefig('test_xx.eps', dpi=300, bbox_inches='tight', transparent=False)
    plt.show()
